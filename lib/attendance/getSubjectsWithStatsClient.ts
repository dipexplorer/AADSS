// Client-side version of getSubjectsWithStats — uses browser Supabase client
// to avoid server action serialization issues when called from useEffect.
...
export async function getSubjectsWithStatsClient(
  semesterId: string,
...
export async function getOverallAttendanceStatsClient(
  semesterId: string,
  studentProfileId: string,
) {
  const { data: subjects, error } = await getSubjectsWithStatsClient(
    semesterId,
    studentProfileId,
  );
