...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    supabase.from("class_sessions")
      .select("id, status, start_time, end_time, subjects(name, semesters(semester_number, programs(name)))")
      .eq("date", todayStr)
      .neq("status", "cancelled")
      .order("start_time")
  ]);
  ...
  // Quick Actions
  const quickActions = [
    { title: "Manage Users", icon: Users, href: "/admin/students", color: "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/40 dark:text-emerald-400" },
    { title: "Eligibility Report", icon: AlertCircle, href: "/admin/defaulters", color: "bg-rose-100 text-rose-600 dark:bg-rose-900/40 dark:text-rose-400" },
    { title: "Schedule Classes", icon: Calendar, href: "/admin/classes", color: "bg-blue-100 text-blue-600 dark:bg-blue-900/40 dark:text-blue-400" },
    { title: "Academic Setup", icon: BookOpen, href: "/admin/sessions", color: "bg-purple-100 text-purple-600 dark:bg-purple-900/40 dark:text-purple-400" },
  ];
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-zinc-50 dark:bg-zinc-900 px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 text-sm font-medium flex items-center gap-3 shadow-inner">
            ...
            <span className="text-zinc-700 dark:text-zinc-300">
              Session: <span className="font-bold text-zinc-900 dark:text-white">{currentSession?.name || 'Not Set'}</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          ...
          <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-2">Total Students</p>
          <div className="flex items-end gap-3">
            <h2 className="text-4xl font-black text-zinc-900 dark:text-zinc-50">{studentCount ?? 0}</h2>
            <span className="text-xs font-medium text-emerald-600 flex items-center mb-1 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full"><TrendingUp className="w-3 h-3 mr-1" /> Active</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          ...
          <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-2">Programs</p>
          <div className="flex items-end gap-3">
            <h2 className="text-4xl font-black text-zinc-900 dark:text-zinc-50">{programCount ?? 0}</h2>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          ...
          <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-2">Subjects Map</p>
          <div className="flex items-end gap-3">
            <h2 className="text-4xl font-black text-zinc-900 dark:text-zinc-50">{subjectCount ?? 0}</h2>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/classes" className="text-sm font-semibold text-primary hover:text-primary/80 transition-colors flex items-center gap-1">
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
            {todayClasses && todayClasses.length > 0 ? (
                  ...
                  return (
                    <div key={cls.id || i} className="p-4 md:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-zinc-900 dark:text-zinc-100">{subj?.name || 'Class Session'}</p>
                          <p className="text-xs font-medium text-zinc-500 mt-0.5 uppercase tracking-wide">
                            {subj?.semesters?.programs?.name} • Sem {subj?.semesters?.semester_number}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-6">
                        ...
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${colorClass}`}>
                          {cls.status}
                        </span>
                      </div>
                    </div>
              ...
              <div className="p-12 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-lg font-bold text-zinc-900 dark:text-zinc-100">No classes scheduled</p>
                <p className="text-sm text-zinc-500 mt-1 max-w-sm">
                  There are no active classes for today in the current academic session.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">Action Center</h2>
          <div className="grid grid-cols-2 gap-3">
            {quickActions.map((action) => (
              ...
              >
                <div className={`p-3.5 rounded-2xl ${action.color} group-hover:scale-110 transition-transform`}>
