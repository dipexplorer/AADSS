...
function StatusBadge({ status }: { status: string }) {
  ...
  > = {
    ...
    approved: {
      label: "Approved",
      cls: "bg-blue-100 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400",
      Icon: CheckCircle,
    },
    rejected: {
      label: "Rejected",
      cls: "bg-red-100 dark:bg-red-950/20 text-red-600 dark:text-red-400",
      Icon: XCircle,
    },
    completed: {
      label: "Reset Completed",
      cls: "bg-green-100 dark:bg-green-950/20 text-green-700 dark:text-green-400",
      Icon: CheckCircle,
    },
  };
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold ${cfg.cls}`}
    >
...
export default function ProfileClient({ profile, resetRequests = [] }: Props) {
  ...
  return (
    <div className="min-h-screen bg-background pt-[60px] pb-24 md:pb-10">
      {/* ── Hero Banner ── */}
      <div className="relative h-36 bg-linear-to-r from-primary/80 via-primary to-primary/60 overflow-hidden">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 -mt-16 space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                ...
                {profile.semester && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-muted/70 rounded-md text-xs font-semibold text-foreground">
                    ...
                    Semester {profile.semester.semester_number}
                  </span>
                )}
                {profile.program && (
            ...
            <div className="flex gap-4 py-2.5 border-b border-border/40 last:border-0">
              <div className="flex-1">
                ...
                <p className="text-sm font-semibold text-foreground">
                  {formatDate(profile.session?.start_date ?? null)}
                </p>
              </div>
              <div className="flex-1">
                <p className="text-[11px] text-muted-foreground font-medium mb-0.5">
                ...
                <p className="text-sm font-semibold text-foreground">
                  {formatDate(profile.session?.end_date ?? null)}
                </p>
              </div>
            </div>
            ...
            <InfoItem
              ...
              value={
                profile.session?.status
                  ? profile.session.status.charAt(0).toUpperCase() +
                    profile.session.status.slice(1)
                  : null
              }
            />
          </div>
        ...
        <div className="bg-card border border-border/60 rounded-2xl shadow-sm p-5">
          <div className="flex items-center gap-2 mb-4">
            <div
              className={`w-7 h-7 rounded-lg flex items-center justify-center ${profile.is_device_registered ? "bg-green-100 dark:bg-green-950/30" : "bg-red-100 dark:bg-red-950/20"}`}
            >
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-sm font-semibold ${profile.is_device_registered ? "text-green-800 dark:text-green-300" : "text-red-700 dark:text-red-400"}`}
              >
                {profile.is_device_registered
                  ? "Primary Device Registered & Locked"
                  : "No Primary Device Registered"}
              </p>
              <p
                className={`text-xs mt-0.5 leading-relaxed ${profile.is_device_registered ? "text-green-700/70 dark:text-green-400/60" : "text-red-600/70 dark:text-red-400/60"}`}
              >
                {profile.is_device_registered
              ...
              {profile.device_id && (
                <p className="mt-2 text-[11px] font-mono text-muted-foreground">
                  ID:{" "}
                  {`${profile.device_id.slice(0, 6)}••••••••${profile.device_id.slice(-5)}`}
                </p>
              )}
            </div>
          </div>
          ...
          {resetRequests.length > 0 && (
            <div className="mt-3">
              ...
              >
                ...
                {showHistory ? "Hide" : "View"} reset history (
              ...
              {showHistory && (
                <div className="mt-2 space-y-2">
                  {resetRequests.map((req) => (
                    <div
                      key={req.id}
                      className="p-3 rounded-xl bg-muted/40 space-y-1.5"
                    >
          ...
          <div className="w-full max-w-md bg-popover border border-border rounded-2xl shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleResetSubmit} className="p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                />
                <p
                  className={`text-[11px] text-right ${reason.length < 20 ? "text-muted-foreground" : "text-green-500"}`}
                >
